...
#include <QDebug>
Combine3DImageYUV::Combine3DImageYUV(QObject* parent):
    Combine3DImageNV12(parent)
{}
bool Combine3DImageYUV::interleavingLine(QImage *srcLeft, QImage *srcRight)
{
    bool ret;
    unsigned char * bgrx;
    int len;
    m_dstlen = m_width*m_height*3;
    ...
    m_rgbToYuv.convertBGRXtoYUV(m_dstbuffer,bgrx,m_width,m_height);

    return true;
}

bool Combine3DImageYUV::interleavingPixelByPixel(QImage *srcLeft, QImage *srcRight)
{
    bool ret;
    unsigned char * bgrx;
    int len;
...
bool Combine3DImageYUV::convertToRawData(QImage *srcImage)
{
    bool ret;
    unsigned char * bgrx;
    int len;
    ...
    bgrx = m_combBgrx.data();
    len = m_combBgrx.datasize();
    m_rgbToYuv.convertBGRXtoYUV(m_dstbuffer,bgrx,width,height);

    return true;
}

bool Combine3DImageYUV::loadBinToImage(QImage *dstImage, unsigned char *buffer, int len)
{
    ...
    if(bgraBuffer){
        yuvToBgr.convertYUVtoBGRX(bgraBuffer,buffer,width,height);
...
bool Combine3DImageYUV::convertLinearToTileBasedBuffer(unsigned char *dstbuffer,
...
void Combine3DImageYUV::convertTilebasedToLinearBuffer(unsigned char *dstbuffer,
                                                        unsigned char *srcbuffer,
                                                        int tlblen,
                                                        int srcw,int srch)
{
