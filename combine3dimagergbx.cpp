...
Combine3DImageRGBX::Combine3DImageRGBX(QObject* parent):Combine3DImage(parent)
{
}
bool Combine3DImageRGBX::interleavingPixelByPixel(QImage *srcLeft, QImage *srcRight)
{
    ...
    for(int i=0;i<height;i++)
    {
       for(int j=0;j<width;j++){
           ...
           m_dstbuffer[8*index]   = c0->r;//r
           m_dstbuffer[8*index+1] = c0->g;//g
           m_dstbuffer[8*index+2] = c0->b;//b
           m_dstbuffer[8*index+3] = 0;//set to 0 in bgrx format
           m_dstbuffer[8*index+4] = c1->r;//r
           m_dstbuffer[8*index+5] = c1->g;//g
           m_dstbuffer[8*index+6] = c1->b;//b
...
bool Combine3DImageRGBX::interleavingLine(QImage *srcLeft, QImage *srcRight)
{
    int width = srcLeft->width();
    int height = srcLeft->height();
    CombineColorBGRA *c0,*c1;//the color of srcleft and srcright
    for(int i=0;i<height;i++)
    {
       for(int j=0;j<width;j++){
           ...
           m_dstbuffer[4*index]   = c0->r;
           m_dstbuffer[4*index+1] = c0->g;
           m_dstbuffer[4*index+2] = c0->b;
           m_dstbuffer[4*index+3] = 0;

           index = (2*i+1)*width+j;
           m_dstbuffer[4*index]   = c1->r;
           m_dstbuffer[4*index+1] = c1->g;
           m_dstbuffer[4*index+2] = c1->b;
           m_dstbuffer[4*index+3] = 0;
...
bool Combine3DImageRGBX::convertToRawData(QImage *srcImage)
{
    CombineColorBGRA* c0;
...
bool Combine3DImageRGBX::convertBinToImage(QImage *dstImage, unsigned char *buffer, int len)
{
...
bool Combine3DImageRGBX::loadBinToImage(QImage *dstImage, unsigned char *buffer, int len)
{
    bool ret = Combine3DImage::loadBinToImage(dstImage,buffer,len);
    int imageSize;
    int width,height;
