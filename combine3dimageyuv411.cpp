...
Combine3DImageYUV411::Combine3DImageYUV411(QObject* p): Combine3DImageI420(p)
{
}
bool Combine3DImageYUV411::interleavingLine(QImage *srcLeft, QImage *srcRight)
{
    bool ret;
    unsigned char * bgrx;
    int len;
    ...
    m_rgbToYuv.convertBGRXtoYUV411(m_dstbuffer,bgrx,m_width,m_height);

    return true;
}

bool Combine3DImageYUV411::interleavingPixelByPixel(QImage *srcLeft, QImage *srcRight)
{
    bool ret;
    unsigned char * bgrx;
    int len;
...
bool Combine3DImageYUV411::convertToRawData(QImage *srcImage)
{
    bool ret;
    unsigned char * bgrx;
    int len;
    ...
    bgrx = m_combBgrx.data();
    len = m_combBgrx.datasize();
    m_rgbToYuv.convertBGRXtoYUV411(m_dstbuffer,bgrx,width,height);

    return true;
}

bool Combine3DImageYUV411::loadBinToImage(QImage *dstImage, unsigned char *buffer, int len)
{
    ...
    if(bgraBuffer){
        yuvToBgr.convertYUV411toBGRX(bgraBuffer,buffer,width,height);
...
bool Combine3DImageYUV411::convertLinearToTileBasedBuffer(unsigned char *dstbuffer,
...
void Combine3DImageYUV411::convertTilebasedToLinearBuffer(unsigned char *dstbuffer,
                                                        unsigned char *srcbuffer,
                                                        int tlblen,
                                                        int srcw,int srch)
{
...
int Combine3DImageYUV411::tileBasedStride(int width)
{
    int u = width/4;
    int a = u;
    if(a%256!=0){
...
int Combine3DImageYUV411::tileBasedHeight(int height)
{
    int uh = height;
    int h = uh;
    if(h%16!=0){
